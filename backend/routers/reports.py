"""
routers/reports.py — Declarative report execution endpoints.

RULES.md:
  LAW 9 : Reports are declarative metadata. No raw SQL in feature code.
           The ReportEngine reads dimension/measure/filter metadata from the
           report_* tables and constructs a safe, parameterised query at runtime.
  LAW 7 : tenant_id comes from the JWT via middleware — never from request body.
  LAW 8 : Every execution is immutable and logged to report_execution_log.
"""

from __future__ import annotations

from typing import Any

import asyncpg
from fastapi import APIRouter, Depends, HTTPException, Request, status
from pydantic import BaseModel

from db import db_conn
from engines.report_engine import ReportEngine

router = APIRouter(prefix="/reports", tags=["reports"])


# ---------------------------------------------------------------------------
# Request / response schemas
# ---------------------------------------------------------------------------

class ReportRunRequest(BaseModel):
    report_code: str
    filters: dict[str, Any] = {}
    export: bool = False


class ReportRunResponse(BaseModel):
    report_code: str
    columns: list[str]
    rows: list[dict]
    row_count: int
    execution_id: str   # UUID serialised as string for JSON compatibility


# ---------------------------------------------------------------------------
# Endpoints
# ---------------------------------------------------------------------------

@router.post("/run", response_model=ReportRunResponse, summary="Execute a declarative report")
async def run_report(
    body: ReportRunRequest,
    request: Request,
    conn: asyncpg.Connection = Depends(db_conn),
) -> ReportRunResponse:
    """
    Execute a report definition stored in report_master.

    The report engine:
      1. Loads report_master, report_dimensions, report_measures, report_filters.
      2. Builds a parameterised query from metadata — NO raw SQL from clients.
      3. Executes the query under the current tenant context (RLS enforced).
      4. Appends an immutable row to report_execution_log (LAW 8).
      5. Returns column labels + row data.

    LAW 9: the caller sends filter values (scalars), never SQL fragments.
    LAW 7: actor_id / tenant_id are sourced from the validated JWT only.
    """
    actor_id = getattr(request.state, "user_id", None)
    role     = getattr(request.state, "role", "app_user")

    try:
        engine = ReportEngine(conn)
        res = await engine.run(
            report_code=body.report_code,
            filters=body.filters,
            actor_id=actor_id,
            role=role,
            export=body.export,
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except PermissionError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc

    columns = (
        [d["label"] for d in res.get("dimensions", [])]
        + [m["label"] for m in res.get("measures", [])]
    )
    # Fallback: derive column names from the actual data row keys when the report
    # has no dimension/measure metadata rows (e.g. reports built on custom SQL views).
    if not columns and res["data"]:
        columns = list(res["data"][0].keys())

    return ReportRunResponse(
        report_code=body.report_code,
        columns=columns,
        rows=res["data"],
        row_count=len(res["data"]),
        execution_id=res["execution_id"],
    )


@router.get("/", summary="List available reports for the current tenant")
async def list_reports(
    conn: asyncpg.Connection = Depends(db_conn),
) -> list[dict]:
    """
    Return all active report definitions visible to the current tenant.
    Tenant isolation is enforced at the DB level via RLS (LAW 7).
    """
    rows = await conn.fetch(
        """
        SELECT report_code, display_name, description, is_active
        FROM   report_master
        WHERE  is_active = true
        ORDER  BY display_name
        """
    )
    return [dict(r) for r in rows]


@router.get("/{report_code}", summary="Describe a report — dimensions, measures, filters")
async def describe_report(
    report_code: str,
    conn: asyncpg.Connection = Depends(db_conn),
) -> dict:
    """Return full metadata for a single report definition."""
    report = await conn.fetchrow(
        "SELECT * FROM report_master WHERE report_code = $1 AND is_active = true",
        report_code,
    )
    if not report:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Report not found")

    report_id = report["report_id"]

    dimensions = await conn.fetch(
        "SELECT * FROM report_dimensions WHERE report_id = $1 ORDER BY sort_order",
        report_id,
    )
    measures = await conn.fetch(
        "SELECT * FROM report_measures WHERE report_id = $1 ORDER BY sort_order",
        report_id,
    )
    filters = await conn.fetch(
        "SELECT * FROM report_filters WHERE report_id = $1 ORDER BY sort_order",
        report_id,
    )

    return {
        "report":     dict(report),
        "dimensions": [dict(d) for d in dimensions],
        "measures":   [dict(m) for m in measures],
        "filters":    [dict(f) for f in filters],
    }
